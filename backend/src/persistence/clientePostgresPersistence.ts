...
export class ClientePostgresRepository {
  async existeCodigo(codigoCliente: string): Promise<boolean> {
    try {
      const resultado = await pool.query(
        'SELECT 1 FROM cliente WHERE codigo_cliente = $1 LIMIT 1',
        [codigoCliente]
      );
  ...
  async insertar(cliente: Cliente): Promise<Cliente> {
    try {
      const resultado = await pool.query(
        `INSERT INTO cliente (codigo_cliente, nombre_cliente, direccion_cliente, telefono_cliente)
         VALUES ($1, $2, $3, $4)
  ...
  async listar(): Promise<Cliente[]> {
    try {
      const resultado = await pool.query(
        `SELECT codigo_cliente AS "codigoCliente",
                nombre_cliente AS "nombreCliente",
                direccion_cliente AS "direccionCliente",
                telefono_cliente AS "telefonoCliente"
         FROM cliente
